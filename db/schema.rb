...
ActiveRecord::Schema[8.0].define(version: 2025_07_21_102153) do
  ...
  create_table "appointments", force: :cascade do |t|
    ...
    t.check_constraint "appointment_type::text = ANY (ARRAY['routine'::character varying, 'follow_up'::character varying, 'emergency'::character varying, 'consultation'::character varying, 'procedure'::character varying, 'surgery'::character varying, 'therapy'::character varying, 'screening'::character varying, 'vaccination'::character varying, 'other'::character varying]::text[])", name: "check_appointments_valid_type"
    ...
    t.check_constraint "status::text = ANY (ARRAY['pending'::character varying, 'confirmed'::character varying, 'in_progress'::character varying, 'completed'::character varying, 'cancelled'::character varying, 'no_show'::character varying, 'rescheduled'::character varying]::text[])", name: "check_appointments_valid_status"
  end
